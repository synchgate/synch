    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-bold text-slate-900 font-['Outfit']">Invoice History</h1>
          <p className="text-slate-500">View and download all your past billing statements.</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Billing Date</th>
            ...
            >
              ...
              <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white sticky top-0 z-20">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-slate-900">{selectedInvoice.id}</h2>
              ...
              {/* Invoice Content */}
              <div className="flex-1 overflow-y-auto p-8 sm:p-12 print:p-0" id="invoice-content" ref={printRef}>
                <div className="max-w-3xl mx-auto space-y-12">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-8">
                    ...
                    <div className="text-right">
                      <div className={`inline-block px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest mb-4 ${
                  ...
                  {/* Billing To */}
                  <div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-4">Billed To</p>
                    <div className="bg-slate-50/50 rounded-2xl p-6 border border-slate-100">
                      <h3 className="text-lg font-bold text-slate-900 mb-1">{selectedInvoice.billingTo.businessName}</h3>
                      <p className="text-sm text-slate-600 mb-2 leading-relaxed max-w-xs">{selectedInvoice.billingTo.address}</p>
                      <p className="text-sm font-medium text-blue-600">{selectedInvoice.billingTo.email}</p>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-left">
                      <thead>
                        <tr className="border-b-2 border-slate-900/5">
                          ...
                          <th className="py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-right">Qty</th>
                          <th className="py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-right">Rate</th>
                          <th className="py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-right">Amount</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {selectedInvoice.items.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td className="py-6 text-right text-sm text-slate-600">{item.quantity}</td>
                            <td className="py-6 text-right text-sm text-slate-600">{formatCurrency(item.rate)}</td>
                            <td className="py-6 text-right text-sm font-bold text-slate-900">{formatCurrency(item.amount)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div className="flex justify-end pt-8">
                    <div className="w-full sm:w-64 space-y-4">
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <span className="text-slate-900 font-bold">{formatCurrency(selectedInvoice.subtotal)}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-slate-500 font-medium">Tax (%)</span>
                        <span className="text-slate-900 font-bold">{formatCurrency(selectedInvoice.tax)}</span>
                      </div>
                      <div className="flex justify-between items-center pt-4 border-t-2 border-slate-900/5">
                        <span className="text-slate-900 font-black uppercase text-xs tracking-wider">Total Amount</span>
                        <span className="text-2xl font-black text-blue-600">{formatCurrency(selectedInvoice.amount)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-blue-50/50 rounded-2xl p-6 border border-blue-100/50 text-center">
                    <p className="text-xs text-blue-800 font-bold mb-1 italic">Thank you for your business!</p>
                    <p className="text-[10px] text-blue-600/70 font-medium">
                      If you have any questions concerning this invoice, contact our billing department at billing@synchgate.io
                    </p>
                  </div>
                </div>
              </div>
